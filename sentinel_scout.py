...
class AnalysisAnalyzer:
    DOMAIN_MAPPINGS = {
        "#crypto": ["btc", "eth", "sol", "xrp", "doge", "ton", "ada", "bybit", "trading", "volume", "tvl", "polymarket", "defi"],
    ...
    def calculate_impact_score(cls, signal: Dict[str, Any], tags: List[str]) -> float:
        ...
        if "#crypto" in tags and signal.get("metric_value", 0) > 100000:
            base_score += 0.2
...
def normalize_signal(source: str, event_id: str, title: str, metric_name: str, metric_val: float, priority: str = "NORMAL") -> Optional[Dict[str, Any]]:
    ...
    payload_hash = generate_payload_hash(source, event_id, clean_title)
...
class DefiLlamaStrategy(IngestionStrategy):
    def collect(self) -> List[Dict[str, Any]]:
        ...
        if res and res.status_code == 200:
            protocols = res.json()
            top_protocols = sorted(protocols, key=lambda x: x.get('tvl', 0), reverse=True)[:3]
            for p in top_protocols:
                norm = normalize_signal(
                    source="defillama",
                    event_id=p.get("slug", p.get("name")),
                    title=f"{p.get('name')} Protocol TVL Spike",
                    metric_name="tvl_usd",
...
class GitHubStrategy(IngestionStrategy):
    def collect(self) -> List[Dict[str, Any]]:
        ...
        res = self.safe_request(url, headers={"Accept": "application/vnd.github.v3+json"})
        if res and res.status_code == 200:
...
class ExecutionGateway:
    ...
    def process_execution(cls, signals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for signal in signals:
            ...
            if signal.get("impact_score", 0.0) >= 0.85 and "#crypto" in signal.get("tags", []):
    ...
    def filter_24h_cycle(signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for s in signals:
            try:
                ...
                if sig_time >= cutoff_time and s.get("impact_score", 0.0) >= 0.4:
                    filtered.append(s)
            except Exception:
                if s.get("impact_score", 0.0) >= 0.4:
                    filtered.append(s)
        return filtered

    @staticmethod
    def sort_by_priority(signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        return sorted(signals, key=lambda x: x.get("impact_score", 0.0), reverse=True)
...
def generate_scout_report(signals: List[Dict[str, Any]], exec_stats: Dict[str, Any]) -> str:
    ...
    formatted_signals = "\n".join([
        f"• [{' '.join(s['tags'])}] [Impact: {s['impact_score']}] {s['title']} | {s['metric_name']}: {s['metric_value']:,.2f}"
...
def send_telegram_message(text: str) -> None:
    """Dispatches structured digest alert to the designated Telegram chat."""
    ...
    try:
        ...
        if e.response is not None and e.response.status_code == 400:
            logger.warning("Telegram Markdown parse error. Falling back to plain text...")
