...
def generate_scout_report(signals: List[Dict[str, Any]], exec_stats: Dict[str, Any]) -> str:
    ...
    Strictly output the response using THIS exact structure and HTML tags for styling:

    <b>📅 Daily Digest — {today_str}</b>

    <b>Direct Impacts</b>
    • [Domain Tag]: [1-sentence event summary] → [Direct outcome/effect]

    <b>Actionable Leverage Points</b>
...
def main() -> None:
    ...
    try:
        ...
        error_msg = f"<b>⚠️ Sentinel Scout Alert</b>\n\nExecution failed: {safe_error}"
